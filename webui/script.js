	...
	function clickCloudIcon(){
		...
		loadJSON('settings', '', '5000', function (response) {
			...
			byId('thermoname').innerHTML = jr.system.host;	
			if(jr.iot.CLon === true){
				window.location = "https://cloud.wlanthermo.de/index.html?api_token=" + jr.iot.CLtoken;
			}else{
	...
	function showPitmaster(){		
		...
		loadJSON('data', '', '3000', function (response) {
			jr = JSON.parse(response);
			pitID = 0;	// Pitmaster1
			clearOption('pitmaster_typ');
			for (var j = 0; j < jr.pitmaster.type.length; j++) {
				byId('pitmaster_typ').options[byId('pitmaster_typ').options.length] = new Option(jr.pitmaster.type[j], jr.pitmaster.type[j]);
			}
			byId('pitmaster_typ').value = jr.pitmaster.pm[pitID].typ;
			...
			byId('pitmaster_channel').value = jr.pitmaster.pm[pitID].channel;
			byId('pitmaster_set').value = jr.pitmaster.pm[pitID].set;
			byId('pitmaster_value').value = jr.pitmaster.pm[pitID].value;
			byId('pitmaster_settings').style.display = "inline";
			showLoader('false');
			showPitmasterProfile(jr.pitmaster.pm[pitID].pid);
			changePitmasterTyp();
		})
	}
	...
	function setPitmaster(){
		...
		loadJSON('data', '', '3000', function (response) {
			jr = JSON.parse(response);
			pitID = 0;	// Pitmaster1
			jr.pitmaster.pm[pitID].typ = byId('pitmaster_typ').value;
			jr.pitmaster.pm[pitID].channel = byId('pitmaster_channel').value;
			jr.pitmaster.pm[pitID].set = byId('pitmaster_set').value;
			jr.pitmaster.pm[pitID].value = byId('pitmaster_value').value
			jr.pitmaster.pm[pitID].pid = byId('pid_profile').value
	...
	function addChannel(number){
		...
		for (i = 0; i < number; i++) {		
			...
			children.forEach(function(item){
				...
				document.getElementById("test1").appendChild(cln);
			});
			document.getElementById("test1").lastElementChild.classList.add("temp_index");
			document.getElementById("test1").lastElementChild.setAttribute('data-channel', i);
		}
		channel_init = true;
	...
	function networklist(){
		loadJSON('networklist', '', '3000', function (response) {
			try {
				...
				jr.Scan = getUnique(jr.Scan,'SSID');				
